...
export const TraineePortal: React.FC<TraineePortalProps> = ({ userName, userEmail }) => {
  const [activeTab, setActiveTab] = useState<'modules' | 'live' | 'quizzes' | 'certificate'>('modules');
  ...
  const handleSubmitQuizAnswer = async (quizId: string, quizTitle: string) => {
    ...
    try {
      // 1. Simpan Jawaban Langsung ke Tabel quiz_answers di Supabase
  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto font-sans space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xs md:text-sm text-gray-300 mt-1 max-w-xl">
              Akses modul kurikulum TOT, gabung sesi mentoring tatap muka, isi kuis refleksi, dan klaim e-Sertifikat kelulusan kamu.
            </p>
          </div>
      ...
      {/* Navigation Sub-Tabs */}
      <div className="flex bg-white p-2 rounded-2xl border border-gray-100 shadow-sm gap-2 overflow-x-auto">
        <button
          type="button"
          onClick={() => setActiveTab('modules')}
          className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 py-3 text-xs font-bold rounded-xl transition cursor-pointer ${
            activeTab === 'modules' ? 'bg-[#0F2C3A] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'
          }`}
        >
          <BookOpen className="w-4 h-4 text-[#F5C748]" />
          <span>Materi LMS ({modules.length})</span>
        </button>
        ...
        <button
          ...
          className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 py-3 text-xs font-bold rounded-xl transition cursor-pointer ${
            activeTab === 'live' ? 'bg-[#0F2C3A] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'
          }`}
        >
          <Video className="w-4 h-4 text-[#F5C748]" />
          <span>Live Mentoring ({sessions.length})</span>
        </button>
        ...
        >
          ...
          <span>Kuis Refleksi ({quizzes.length})</span>
        </button>
        ...
        <button
          ...
          className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 py-3 text-xs font-bold rounded-xl transition cursor-pointer ${
            activeTab === 'certificate' ? 'bg-[#0F2C3A] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'
          }`}
        >
